			     ...
			     int maxlnb, enum dt_bufs_type rw)
{
	...
	s64 delta_ms;
	gfp_t gfp_mask;
	dmu_flags_t dmu_flags = DMU_READ_PREFETCH;
	 ...
	 * page aligned for reads. If alignment check fails, we mark the lnb
	 * and fall back to uncached io.
	 */
	if (osd_dmu_use_direct(osd, 0, off, len, rw, obj, lnb)) {
			      ...
			      int maxlnb, enum dt_bufs_type rw)
{
	 ...
	 */
	if (osd_dmu_use_direct(osd, bs, off, len, rw, obj, lnb) &&
	    bs >= osd->od_max_blksz) {
			...
			struct thandle *th, __u64 user_size)
{
	...
	dmu_flags_t dmu_flags = DB_RF_MUST_SUCCEED;
